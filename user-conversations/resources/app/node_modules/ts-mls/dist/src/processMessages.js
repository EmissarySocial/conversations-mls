...
export async function processPrivateMessage(params) {
    ...
    if (result.content.content.contentType === contentTypes.application) {
        ...
        const { newState, actionTaken, consumed } = await processCommit(updatedState, result.content, "mls_private_message", pskSearch, cb, auth, clientConfig, cipherSuite); //todo solve with types
        return {
            kind: "newState",
            newState,
            actionTaken,
...
export async function processPublicMessage(params) {
    ...
    if (content.content.contentType === contentTypes.proposal) {
        ...
        return processCommit(state, content, "mls_public_message", pskSearch, callback, auth, clientConfig, cipherSuite); //todo solve with types
    }
}
async function processCommit(state, content, wireformat, pskSearch, callback, authService, clientConfig, cs) {
    ...
    const senderLeafIndex = content.content.sender.senderType === senderTypes.member ? toLeafIndex(content.content.sender.leafIndex) : undefined;
